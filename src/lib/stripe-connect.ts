 ...
 */
export const createStripeConnectLinkFn = createServerFn({ method: "POST" })
  .handler(async () => {
 ...
 */
export const getStripeConnectStatusFn = createServerFn({ method: "GET" })
  .handler(async () => {
    const request = getRequest();
    const session = await auth.api.getSession({ headers: request.headers });
    ...
    try {
      ...
      return { connected: false, detailsSubmitted: false, chargesEnabled: false, accountId: tenant.stripeConnectAccountId };
    }
  });
